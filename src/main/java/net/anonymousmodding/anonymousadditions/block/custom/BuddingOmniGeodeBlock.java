...
public class BuddingOmniGeodeBlock extends AmethystBlock {
    public static final int GROWTH_CHANCE = 5;
    private static final Direction[] DIRECTIONS = Direction.values();

    public BuddingOmniGeodeBlock(BlockBehaviour.Properties pProperties) {
        super(pProperties);
    }
    ...
    public void randomTick(BlockState pState, ServerLevel pLevel, BlockPos pPos, RandomSource pRandom) {
        if (pRandom.nextInt(5) == 0) {
            ...
            if (canClusterGrowAtState($$6)) {
                newBlock = ModBlocks.SMALL_OMNIGEODE_CLUSTER.get();
            } else if ($$6.is(ModBlocks.SMALL_OMNIGEODE_CLUSTER.get()) && $$6.getValue(AmethystClusterBlock.FACING) == $$4) {
                newBlock = ModBlocks.MEDIUM_OMNIGEODE_CLUSTER.get();
            } else if ($$6.is(ModBlocks.MEDIUM_OMNIGEODE_CLUSTER.get()) && $$6.getValue(AmethystClusterBlock.FACING) == $$4) {
                newBlock = ModBlocks.LARGE_OMNIGEODE_CLUSTER.get();
            } else if ($$6.is(ModBlocks.LARGE_OMNIGEODE_CLUSTER.get()) && $$6.getValue(AmethystClusterBlock.FACING) == $$4) {
                newBlock = ModBlocks.OMNIGEODE_CLUSTER.get();
            }
